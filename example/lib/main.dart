...
class MultiSplitViewExampleState extends State<MultiSplitViewExample> {
  ...
  Widget build(BuildContext context) {
    Widget buttons = Container(
        ...
        child: Wrap(crossAxisAlignment: WrapCrossAlignment.center, spacing: 10, runSpacing: 10, children: [
          ...
          ElevatedButton(onPressed: _controller.areasCount != 0 ? _onRemoveFirstButtonClick : null, child: const Text('Remove first')),
          Checkbox(
              value: _pushDividers,
    ...
    if (_controller.areasCount != 0) {
      ...
      content = Padding(padding: const EdgeInsets.all(16), child: MultiSplitViewTheme(data: MultiSplitViewThemeData(dividerPainter: DividerPainters.grooved2()), child: multiSplitView));
    } else {
  ...
  _onDividerTap(int dividerIndex) {
    ...
    // Update the flex factor
    _updateFlex(index: dividerIndex, controller: _controller);
  }
  ...
  void _updateFlex({required int index, required MultiSplitViewController controller}) {
    final area = controller.getArea(index);
    AreaHelper.setFlex(area: area, flex: 1);
    controller.forceNotifyListeners();
  }

  void _updateSize({required int index, required MultiSplitViewController controller}) {
    final area = controller.getArea(index);
    AreaHelper.setSize(area: area, size: 200);
    controller.forceNotifyListeners();
  }
}

class ColorWidget extends StatelessWidget {
  const ColorWidget({Key? key, required this.color, required this.onRemove, required this.area}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    Widget info = Center(child: Container(color: const Color.fromARGB(200, 255, 255, 255), padding: const EdgeInsets.fromLTRB(3, 2, 3, 2), child: Wrap(runSpacing: 5, spacing: 5, crossAxisAlignment: WrapCrossAlignment.center, children: children)));

    return InkWell(onTap: () => onRemove(area.index), child: Container(color: color, child: Stack(children: [const Placeholder(color: Colors.black), info])));
  }
}
